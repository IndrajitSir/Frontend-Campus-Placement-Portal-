...
function InterviewRoom({ user, isInterviewer, roomId, language, setLanguage }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex h-[calc(100vh-6rem)] flex-col gap-4">
      ...
      {/* ── Header ── */}
      <div className="relative shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-white px-5 py-3 shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-base font-bold text-slate-900">Live Interview</h2>
              <div className="flex items-center gap-2 text-[11px] text-slate-400">
                <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] text-indigo-600">{roomId?.slice(0, 8)}…</code>
                <button onClick={handleCopyIdToClipboard} className="flex cursor-pointer items-center gap-0.5 rounded border border-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500 transition hover:border-indigo-300 hover:text-indigo-600">
          ...
          <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center gap-1">
                ...
                <div className="flex -space-x-1">
                  {participants.slice(0, 4).map((p, i) => (
                    <div key={p.userId || i} title={p.name} className="flex h-5 w-5 items-center justify-center rounded-full bg-indigo-100 text-[8px] font-bold text-indigo-600 ring-1.5 ring-white">
            ...
            {timerStarted ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2.5 py-0.5 font-mono text-[11px] font-bold text-indigo-600">
      ...
      <div className="flex min-h-0 flex-1 gap-4">
        ...
        <div className="min-w-0 flex-1 overflow-hidden rounded-2xl border border-slate-800 bg-[#1e1e1e] shadow-lg">
          <CodeEditor
