...
// Icons
import { Eye, Trash, FileCheck, Briefcase, Building2, CalendarDays, Download, LoaderCircle } from 'lucide-react';
// Dialog Boxes
...
function ShortlistedApplications() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        setLoading(true);
        const res = await window.fetch(`${API_URL}/api/v3/applications/shortlisted-candidates?page=${page}&limit=9`, {
          method: "GET",
          credentials: "include",
          headers: {
  ...
  const handleExportCSV = async () => {
    try {
      setExporting(true);
      const res = await fetch(`${API_URL}/api/v3/applications/export?status=shortlisted`, {
        method: "GET",
        credentials: "include",
        headers: {
          'Authorization': `Bearer ${accessToken}`
      ...
      a.href = url;
      a.download = "shortlisted-applications.csv";
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      toast.success("Shortlisted applications exported as CSV");
    } catch (err) {
      console.error("Failed to export shortlisted applications", err);
    ...
    <div className="w-full space-y-6 p-6 pt-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-display text-lg font-bold text-slate-900 dark:text-slate-100">Shortlisted Applications</h2>
          <span className="rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-600 dark:bg-amber-500/10 dark:text-amber-300">
            {applications.length}
          </span>
        </div>
        {canExport && (
      ...
      {!loading && applications.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-50 text-amber-300 dark:bg-amber-500/10 dark:text-amber-400/60">
            <FileCheck className="h-7 w-7" />
          </span>
          <p className="mt-3 text-sm font-medium text-slate-400 dark:text-slate-500">No shortlisted candidates yet.</p>
        </div>
      )}
              ...
              <Card className="card-elevate h-full rounded-2xl border-slate-200/80 p-5 transition-all duration-200 dark:border-white/10 dark:bg-white/[0.04]">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-amber-50 text-sm font-bold text-amber-600 ring-2 ring-amber-100 dark:bg-amber-500/10 dark:text-amber-300 dark:ring-amber-500/20">
                  ...
                  <span className="pill-shortlisted inline-flex shrink-0 items-center text-[10px] font-semibold">
                    Shortlisted
                  </span>
                </div>
                ...
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <TooltipProvider delayDuration={200}>
                    <Tooltip>
                      ...
                      <TooltipContent side="top"><p>Remove from shortlist</p></TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
              </Card>
