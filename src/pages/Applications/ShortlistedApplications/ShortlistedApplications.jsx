...
function ShortlistedApplications() {
  ...
  useEffect(() => {
    async function getData() {
      const res = await fetch(`${API_URL}/api/v1/applications/shortlisted-candidates`, {
        method: "GET",
        headers: {
            ...
            <Card key={application._id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div key={application._id} className=" p-4 rounded shadow-md bg-white">
                  <h3 className="text-xl font-semibold"> Shortlisted {application.user_id.name} for this {application.placement_id.job_title} at {application.placement_id.company_name}</h3>
