...
// Icons
import { Eye, Trash, XCircle, Briefcase, Building2, CalendarDays, LoaderCircle } from 'lucide-react';
// Dialog Boxes
  ...
  return (
    <div className="w-full space-y-6 p-6 pt-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-display text-lg font-bold text-slate-900">Rejected Applications</h2>
      ...
      {loading && (
        <div className="flex items-center justify-center gap-2 py-12 text-sm text-slate-400">
          <LoaderCircle className="h-4 w-4 animate-spin text-red-500" /> Loading applications…
        </div>
      )}

      {!loading && applications.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-red-50 text-red-300">
            <XCircle className="h-7 w-7" />
          </span>
          <p className="mt-3 text-sm font-medium text-slate-400">No rejected candidates yet.</p>
        </div>
      )}
            ...
            <Card className="card-elevate h-full border-slate-200/80 p-5">
              <div className="flex items-start justify-between gap-2">
                <div className="flex min-w-0 items-center gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-red-50 text-sm font-bold text-red-600 ring-2 ring-red-100">
                ...
                <span className="inline-flex shrink-0 items-center rounded-full bg-red-50 px-2 py-0.5 text-[10px] font-semibold text-red-600">
                  Rejected
                </span>
              </div>
