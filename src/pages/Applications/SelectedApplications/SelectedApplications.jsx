...
// Icons
import { Eye, Trash, CheckCircle, Briefcase, Building2, CalendarDays, Download, LoaderCircle } from 'lucide-react';
// Dialog Boxes
...
function SelectedApplications() {
  ...
  const handleExportCSV = async () => {
    try {
      setExporting(true);
      const res = await fetch(`${API_URL}/api/v3/applications/export?status=selected`, {
        method: "GET",
        credentials: "include",
        headers: {
          'Authorization': `Bearer ${accessToken}`
      ...
      a.href = url;
      a.download = "selected-applications.csv";
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      toast.success("Selected applications exported as CSV");
    } catch (err) {
      console.error("Failed to export selected applications", err);
    ...
    <div className="w-full space-y-6 p-6 pt-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-display text-lg font-bold text-slate-900 dark:text-slate-100">Selected Applications</h2>
      ...
      {!loading && applications.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-3 text-sm font-medium text-slate-400 dark:text-slate-500">No selected candidates yet.</p>
        </div>
      )}
              ...
              <Card className="card-elevate h-full rounded-2xl border-slate-200/80 p-5 transition-all duration-200 dark:border-white/10 dark:bg-white/[0.04]">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-sm font-bold text-emerald-600 ring-2 ring-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-500/20">
                  ...
                  <span className="pill-selected inline-flex shrink-0 items-center text-[10px] font-semibold">
                    Selected
                  </span>
                </div>
