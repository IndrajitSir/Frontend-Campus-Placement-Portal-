...
function SelectedApplications() {
  ...
  useEffect(() => {
    async function getData() {
      const res = await fetch(`${API_URL}/api/v1/applications/selected-candidates`, {
        method: "GET",
        headers: {
      ...
      console.log("Selected Candidates: ", response?.data);
    }
    getData();
  }, [])
  return (
            ...
            <Card key={application._id}>
              <CardContent className="p-4 flex justify-between items-center">
                <div key={application._id} className=" p-4 rounded shadow-md bg-white">
                  <h3 className="text-xl font-semibold">{application.user_id.name} got selected for this {application.placement_id.job_title} at {application.placement_id.company_name}</h3>
