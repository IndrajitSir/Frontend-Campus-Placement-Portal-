...
function SearchDialog({ data, searchCriteria, onQuery, placeholderValue }) {
    ...
    const handleSearchChange = (e) => {
        ...
        const results = data.filter(p =>
            p[searchCriteria[0]].toLowerCase().includes(value.toLowerCase()) ||
            p[searchCriteria[1]].toLowerCase().includes(value.toLowerCase())
        );
    ...
    const handleSuggestionClick = (user) => {
        setSearchQuery(user[searchCriteria[0]]);
        onQuery(user);
        handleOverlayClose();
    };
                                ...
                                {Array.isArray(filteredResults) && filteredResults.length > 0 ? (
                                    filteredResults.map((user, i) => (
                                        <div key={i} onClick={() => handleSuggestionClick(user)}
                                            ...
                                            <strong>{user[searchCriteria[0]]}</strong> -- {user[searchCriteria[1]]}
                                        </div>
                                    ))
                                ) : (
